...
import { submitCode, onCodeResult, offCodeResult, onBattlePlayerWon, offBattlePlayerWon, onBattleEnded, offBattleEnded } from '@/lib/socket'
...
export default function BattleArena({
  ...
  useEffect(() => {
    ...
    const handleCodeResult = (data: { userId: string; result: any }) => {
      console.log('[v0] Code result received:', data)
      setCodeResults((prev) => ({
        ...prev,
  ...
  const handlePasswordSubmit = async (password: string) => {
    ...
    try {
      // The password is actually the room code
      // In a real scenario, you'd verify it with the backend
      if (battle.roomCode === password) {
        setShowPasswordPrompt(false)
      } else {
        setPasswordError('Invalid battle code')
      }
    } catch (error) {
  ...
  const handleCodeSubmit = async (code: string) => {
    ...
    try {
      console.log('[v0] Submitting code for battle:', battle.bid)
      const result = await submitCode(battle.bid, battle.challenge.language, code)
      console.log('[v0] Submit code response:', result)
      ...
      console.error('[v0] Error submitting code:', error)
  ...
  // Show password prompt if battle is private and room code not matched
  if (battle.visibility === 'PRIVATE' && showPasswordPrompt) {
    return (
      <PasswordPrompt
        battleId={battle.bid}
      ...
      <div className="border-b border-border pb-4">
        ...
        <div className="grid grid-cols-3 gap-4 mt-4">
          ...
          <div>
            ...
            <p className="text-sm font-semibold text-card-foreground">{battle.challenge.timeLimitMin}s</p>
          </div>
          <div>
