...
export default function BattleCard({ battle }: { battle: Battle }): React.JSX.Element {
  ...
  const handlePasswordSubmit = async (password: string) => {
    ...
    try {
      // the room code lives on the server only, an invalid one is rejected there
      await joinBattle(battle.bid, password)
      setShowPasswordPrompt(false)
    } catch (error) {
      setPasswordError((error as Error).message || "Failed to join battle")
    } finally {
      setIsJoining(false)
    }
  }
  ...
  const submitJoinBattle = async () => {
    ...
    setJoinError('')
    try {
      await joinBattle(battle.bid)
    } catch (error) {
      setJoinError((error as Error).message || "Failed to join battle")
    } finally {
      setIsJoining(false)
    }
  }
