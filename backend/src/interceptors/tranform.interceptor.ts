...
@Injectable()
export class TranformInterceptor<T> implements NestInterceptor { // like inherits from NestInterceptor, and T is a generic type parameter that allows the interceptor to work with any type of data.
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> { // context: Gives you access to everything about the current request , and next: Represents the rest of the pipeline.
    ...
    return next.handle().pipe(
      map((data: T) => {
        const statusCode = context.switchToHttp().getResponse().statusCode ?? 200; // ?? use the left side if it's not null/undefined, otherwise use the right side.
        return {
          statusCode,
          message: STATUS_CODES[statusCode] ?? 'Success',  // 200 → 'OK', 201 → 'Created'...
          data,
        };
      }));
