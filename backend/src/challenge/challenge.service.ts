...
export class ChallengeService {
  ...
  async deleteChallenge(
    ...
    if (
      ...
      throw new ForbiddenException(
        'Published challenges can only be deleted by an admin',
      );
    }

    if (challenge._count.battles > 0) {
      throw new ConflictException(
        'This challenge has been used in battles and cannot be deleted',
      );
    }
  ...
  async getChallengeById(challengeId: string, userId: string) {
    ...
    // expectedOutput is the answer: only its creator may read it back
    return this.databaseService.challenge.findUniqueOrThrow({
      where: { cid: challengeId },
      omit: { expectedOutput: challenge.createdById !== userId },
    });
  }

  async getAllChallenges() {
    return this.databaseService.challenge.findMany({
      omit: { expectedOutput: true },
    });
  }
